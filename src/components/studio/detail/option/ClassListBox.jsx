import React, { useEffect, useRef, useState } from 'react';

import { useRouter } from 'next/router';

import { useQuery } from 'react-query';
import ProductService from 'src/service/ProductService';
import { addComma } from 'src/constants';

import { SClassListBox } from './ClassListBox.styled';

const ClassListBox = ({ id }) => {
  const router = useRouter();

  const showRef = useRef();

  const { data, isLoading } = useQuery(['class-list', id], () => ProductService.getLessons(id));

  const [isOpen, setIsOpen] = useState(false);

  const [mainHeight, setMainHeight] = useState(1000);

  // 높이 조절
  useEffect(() => {
    if (showRef.current && !isLoading) {
      setMainHeight(showRef.current.clientHeight);
    }
  }, [showRef, isOpen, isLoading]);

  // 메뉴 열고 닫기
  const onClickShowList = () => {
    setIsOpen((prev) => !prev);
  };

  const onClickGoLesson = (e) => {
    setIsOpen(false);
    router.push(`/lesson/${e}`);
  };

  return (
    <SClassListBox>
      <nav
        className="class-list-bottombar-container"
        style={{
          transform: `translateX(-50%) translateY(${isOpen ? 0 : mainHeight}px)`,
        }}
      >
        <div className="class-list-bottombar-box">
          <div className="class-list-bottombar-show-header" onClick={onClickShowList}>
            <p className="class-list-bottombar-show-title">예약하기</p>
            <i className={`class-list-bottombar-show-icon ${isOpen ? 'active' : ''}`} />
          </div>
        </div>

        <div className="class-list-show-container" ref={showRef}>
          <ClassList data={data} isLoading={isLoading} onClickGoLesson={onClickGoLesson} />
        </div>
      </nav>
    </SClassListBox>
  );
};

const ClassList = ({ data, isLoading, onClickGoLesson }) => {
  return (
    <React.Fragment>
      {!isLoading &&
        (data?.value?.length > 0 ? (
          <ul className="class-item-list-container">
            {data?.value
              ?.sort((a, b) => (a.sortOrder < b.sortOrder ? 1 : -1))
              ?.map((item) => (
                <li key={item?.id} className={`class-item ${item?.isSoldOut ? 'isSoldOut' : ''}`} onClick={() => onClickGoLesson(item?.id)}>
                  <div className="class-title-container">
                    <p>{item?.title || ''}</p>

                    {item?.isSoldOut && <div className="item-impossible">품절</div>}
                  </div>

                  <div className="class-price-container">
                    <p>{addComma(item?.minPrice || 0)}원</p>

                    <div className="class-arrow-icon" />
                  </div>
                </li>
              ))}
          </ul>
        ) : (
          <p className="empty-text">등록된 수업이 없습니다.</p>
        ))}

      {isLoading && <ClassFallbackLoading />}
    </React.Fragment>
  );
};

const ClassFallbackLoading = () => {
  return (
    <div className="class-fallback-loading">
      <i className="icons svg-loading" />
    </div>
  );
};

export default ClassListBox;
