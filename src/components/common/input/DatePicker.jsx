...
import { SDatePicker } from './DatePickerStyles';
import moment from 'moment';

const WEEK = ['일', '월', '화', '수', '목', '금', '토'];
const FORMAT = 'YYYY-MM-DD';

const DatePicker = ({ value, onChange, selectList, allSelect }) => {
  ...
  useEffect(() => {
    if (selectList?.length) {
      ...
      selectList.forEach((item) => {
        temp.push(dateFormat(item?.departureDate));
      });
  ...
  const createCalender = (_year, _month, _days, _insert) => {
    ...
    _days.forEach((day, idx) => {
      ...
      //   선택 데이터
      if (formatDate === selectFormatDate) {
        active = true;
      }
      ...
      //   오늘 날짜
      if (formatDate === dateFormat(nowDate)) {
        now = true;
      }
      ...
      td.push(
        <td
          ...
          ${active ? '_active' : ''} 
          ${empty ? '_empty' : ''}
          ${disabled ? '_disabled' : '_un_disabled'}
          `}
        >
          <div>{day}</div>
        </td>,
  ...
  return (
    <SDatePicker>
      <div className="date-picker-container">
        ...
        <div className="datepicker">{renderCalender(currentYear)}</div>
      </div>
    </SDatePicker>
  );
};
